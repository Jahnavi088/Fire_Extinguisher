...
const fetchByType = (type) => {
  const params = { module_id: 31, limit: 200 }; // Assuming module_id 31 for Sprinklers
  ...
  return (
    <div className="fe-pagination">
      <span className="fe-page-info">
        Showing <strong>{start}–{end}</strong> of <strong>{total}</strong> sprinklers
      </span>
...
const ReadinessBar = ({ score }) => {
  ...
  return (
    <>
      <div className="fe-readiness-label">System Integrity Score</div>
  ...
  const load = async () => {
    try {
      ...
      const [sum, alertsSum, alertsData] = await Promise.all([
        ApiService.getModuleSummary(31),                       // /modules/31/summary
        ApiService.getAlertsSummary(),                         // /alerts/summary
        ApiService.getAlerts({ module_id: 31, limit: 100 }),   // /alerts?module_id=31
      ]);
      setSummary(sum);
                  ...
                  return (
                    <div key={a.id || i} className="fe-alert-row" style={{ '--alert-color': c }}>
                      ...
                      <div className="fe-alert-meta">
                        ...
                        <div className="fe-alert-type">{a.equipment_type || 'Sprinkler'}</div>
                      </div>
                    </div>
                  );
                });
    ...
    <div className="fe-page">
      ...
      <div className="fe-header">
        ...
        <div className="fe-header-info">
          ...
          <div className="fe-header-sub">{u.equipment_type || 'Sprinkler'} · {u.location_name}</div>
        </div>
        <span
          ...
          <div className="fe-detail-card fe-full">
            ...
            <div className="fe-identity-grid">
              {[
                ...
                { label: 'Type', val: u.equipment_type || 'Sprinkler' },
