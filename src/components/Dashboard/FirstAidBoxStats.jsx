...
const condColor = (v) =>
  v === 'OK' || v === 'COMPLETE' || v === 'SEALED' ? '#10b981'
...
const KPI_CARDS = [
  ...
  { type: 'active', label: 'Complete', icon: '✅', color: '#10b981', key: 'active' },
...
const fetchByType = async (type, moduleId = 15) => {
  if (type === 'active') {
...
/* ══════════════════════════════════════════════════════════════════════════ */
const FirstAidBoxStats = ({ module, onBack, onRaiseWorkOrder }) => {
  const modId = module?.module_id || 45;
            ...
            {summary && (
              <>
                <div style={{ height: 160, width: '100%', marginTop: 5 }}>
                  <ResponsiveContainer>
                    <BarChart data={[
                      { name: 'Complete', val: (summary.active || 0) + (summary.upcoming || 0), color: '#10b981' },
