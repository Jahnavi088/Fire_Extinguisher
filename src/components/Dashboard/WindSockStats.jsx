...
const condColor = (v) =>
  v === 'OK' || v === 'GOOD' || v === 'STABLE' ? '#28a745'
    : (v === 'WORN' || v === 'FADED' || v === 'DIRTY') ? '#FF9800'
      : (v === 'TORN' || v === 'MISSING' || v === 'DAMAGED') ? '#dc3545'
        : '#666';
...
const KPI_CARDS = [
  ...
  { type: 'expired', label: 'Damaged/Torn', icon: '⌛', color: '#8b5cf6', key: 'expired' },
...
const WindSockStats = ({ onBack }) => {
  ...
  const load = async () => {
    try {
      ...
      console.error('API Load failed for Wind Sock, using fallback:', err);
      setSummary({ total: 6, active: 5, upcoming: 1, needs_service: 0, expired: 0, due_inspection: 0, readiness_score: 95 });
      setAlertsSummary({ total_alerts: 0, level_1: { count: 0, label: 'Low', description: 'Minor' }, level_2: { count: 0, label: 'Med', description: 'Action' }, level_3: { count: 0, label: 'High', description: 'Critical' } });
      setTopAlerts([]);
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        const mock = Array.from({ length: 3 }).map((_, i) => ({
          id: `ws_${i}`,
          sos_code: `WS-0${i + 1}`,
          equipment_type: 'Wind Sock',
          location_name: `Boundary Wall ${i + 1}`,
          building_name: 'External Area',
          readiness_score: 100,
        ...
        <div className="fe-header">
          ...
          <div className="fe-header-search">
            <div className="fe-search-box">
              ...
              <input type="text" placeholder="Search wind sock code..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="fe-search-input" />
            </div>
          </div>
        </div>
        ...
        <div className="fe-panels">
          <div className="fe-panel">
            ...
            <div className="fe-alert-list">
              {topAlerts.length === 0 ? <div className="fe-empty">No active wind sock alerts.</div> : topAlerts.map((a, i) => (
        ...
        <div className="fe-detail-grid">
          <div className="fe-detail-card fe-full"><div className="fe-section-title">Unit Identity</div>
            <div className="fe-identity-grid">
              ...
              <InfoRow label="Mount Type" val={u.mount_type || 'Pillar'} />
          ...
          <div className="fe-detail-card"><div className="fe-section-title">🔧 Fabric Integrity</div>
            <div className="fe-condition-pills">
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.fabric_status || 'OK') + '44', background: condColor(u.fabric_status || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Fabric</span><span className="fe-condition-pill-value" style={{ color: condColor(u.fabric_status || 'OK') }}>{u.fabric_status || 'NORMAL'}</span>
              </div>
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.cage_status || 'OK') + '44', background: condColor(u.cage_status || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Cage/Ring</span><span className="fe-condition-pill-value" style={{ color: condColor(u.cage_status || 'OK') }}>{u.cage_status || 'NORMAL'}</span>
              </div>
            </div>
