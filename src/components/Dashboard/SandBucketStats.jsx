...
const condColor = (v) =>
  v === 'OK' || v === 'GOOD' || v === 'STABLE' || v === 'FULL' || v === 'DRY' ? '#10b981'
    : (v === 'WORN' || v === 'FADED' || v === 'DIRTY' || v === 'LOW') ? '#FF9800'
      : (v === 'TORN' || v === 'MISSING' || v === 'DAMAGED' || v === 'EMPTY' || v === 'WET') ? '#dc3545'
        : '#666';
...
const KPI_CARDS = [
  ...
  { type: 'expired', label: 'Damaged/Empty', icon: '⌛', color: '#8b5cf6', key: 'expired' },
  { type: 'due-inspection', label: 'Due Inspection', icon: '🚨', color: '#dc3545', key: 'due_inspection' },
];

const fetchByType = async (type, moduleId = 63) => {
  if (type === 'active') {
...
const Spinner = () => (
  <div className="fe-spinner">
    ...
    <span className="fe-spinner-text">Loading sand bucket data…</span>
  </div>
);
...
/* ══════════════════════════════════════════════════════════════════════════ */
const SandBucketStats = ({ module, onBack, onRaiseWorkOrder }) => {
  const modId = module?.module_id || 63;
    ...
    return (
      <div className="fe-page">
        <div className="fe-header">
          ...
          <div className="fe-header-info"><div className="fe-header-title">Sand Bucket Fleet Monitor</div></div>
          ...
          <div className="fe-header-search">
            <div className="fe-search-box">
              ...
              <input type="text" placeholder="Search sand bucket code..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="fe-search-input" />
            </div>
          </div>
        </div>
        ...
        <div className="fe-panels">
          <div className="fe-panel">
            <div className="fe-panel-title">🔔 Physical Condition Alerts <span className="fe-panel-title-count">{totalAlerts}</span></div>
            <div className="fe-alert-list">
              {topAlerts.length === 0 ? <div className="fe-empty">No active sand bucket alerts.</div> : topAlerts.map((a, i) => (
                ...
                <div style={{ height: 160, width: '100%', marginTop: 5 }}>
                  <ResponsiveContainer>
                    <BarChart data={[
                      ...
                      { name: 'Needs Refill', val: summary.needs_service, color: '#f59e0b' },
    ...
    <div className="fe-page">
      <div className="fe-header">
        ...
        <div className="fe-header-info">
          ...
          <div className="fe-header-sub">{u.equipment_type || 'Sand Bucket'} · {u.location_name}</div>
        </div>
        {onRaiseWorkOrder && (
          <button
        ...
        <div className="fe-detail-grid">
          ...
          <div className="fe-detail-card fe-full"><div className="fe-section-title">Unit Identity</div>
            <div className="fe-identity-grid">
              ...
              <InfoRow label="Capacity" val={u.capacity || '10 kg'} />
          ...
          <div className="fe-detail-card"><div className="fe-section-title">📅 Key Dates</div>
            ...
            <InfoRow label="Last Check" val={fmt(u.last_inspection_date)} />
            <InfoRow label="Next Due" val={fmt(u.next_inspection_due)} color="#FF9800" />
          </div>
          <div className="fe-detail-card"><div className="fe-section-title">🔧 Material Integrity</div>
            <div className="fe-condition-pills">
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.sand_status || 'OK') + '44', background: condColor(u.sand_status || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Sand Level</span><span className="fe-condition-pill-value" style={{ color: condColor(u.sand_status || 'OK') }}>{u.sand_status || 'FULL'}</span>
              </div>
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.bucket_condition || 'OK') + '44', background: condColor(u.bucket_condition || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Bucket</span><span className="fe-condition-pill-value" style={{ color: condColor(u.bucket_condition || 'OK') }}>{u.bucket_condition || 'GOOD'}</span>
              </div>
            </div>
...
export default SandBucketStats;
