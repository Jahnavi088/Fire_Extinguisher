...
const statusColor = (v) =>
  v === 'ON_DUTY' || v === 'READY' || v === 'PRESENT' ? '#10b981'
...
const KPI_CARDS = [
  ...
  { type: 'active', label: 'On Duty', icon: '✅', color: '#10b981', key: 'active' },
...
const fetchByType = async (type) => {
  if (type === 'active') {
    const [activeData, upcomingData] = await Promise.all([
      ApiService.getEquipment({ module_id: 22, limit: 500, status: 'active' }),
      ApiService.getEquipment({ module_id: 22, limit: 500, status: 'upcoming' }),
    ]);
    return {
  ...
  const params = { module_id: 22, limit: 500 };
...
/* ══════════════════════════════════════════════════════════════════════════ */
const ShiftVolunteerStats = ({ module, onBack, onRaiseWorkOrder }) => {
  ...
  const load = async () => {
    try {
      ...
      const [sum, alertsSum, alertsData] = await Promise.all([
        ...
        ApiService.getAlertsSummary({ module_id: 22 }),
            ...
            {summary && (
              <>
                <div style={{ height: 160, width: '100%', marginTop: 5 }}>
                  <ResponsiveContainer>
                    <BarChart data={[
                      { name: 'On Duty', val: (summary.active || 0) + (summary.upcoming || 0), color: '#10b981' },
                      ...
                      <Bar dataKey="val" radius={[4, 4, 0, 0]} barSize={55}>
                        {[{ color: '#10b981' }, { color: '#f59e0b' }].map((entry, index) => <Cell key={index} fill={entry.color} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
