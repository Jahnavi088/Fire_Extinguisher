...
const statusColor = (v) =>
  v === 'CERTIFIED' || v === 'ACTIVE' || v === 'ON_DUTY' ? '#10b981'
...
const KPI_CARDS = [
  ...
  { type: 'active', label: 'On Shift Now', icon: '🕒', color: '#10b981', key: 'active' },
  ...
  const load = async () => {
    try {
      ...
      const [sum, alertsSum, alertsData] = await Promise.all([
        ...
        ApiService.getAlertsSummary({ module_id: 23 }),
            ...
            {summary && (
              <>
                <div style={{ height: 160, width: '100%', marginTop: 5 }}>
                  <ResponsiveContainer>
                    <BarChart data={[
                      { name: 'Certified', val: (summary.active || 0) + (summary.upcoming || 0), color: '#10b981' },
