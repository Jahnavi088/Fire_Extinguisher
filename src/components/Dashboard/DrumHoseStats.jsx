...
const KPI_CARDS = [
  { type: 'all', label: 'Total Units', icon: '🛢️', color: '#3b82f6', key: 'total' },
  ...
  return (
    <div className="fe-pagination">
      <span className="fe-page-info">
        Showing <strong>{start}–{end}</strong> of <strong>{total}</strong> drum hose reels
      </span>
...
const ReadinessBar = ({ score }) => {
  ...
  return (
    <>
      <div className="fe-readiness-label">Fleet Compliance Score</div>
...
const DrumHoseStats = ({ module, onBack, onRaiseWorkOrder }) => {
  ...
  const load = async () => {
    try {
      ...
      console.error('API Load failed for Drum Hose Reels, using fallback data:', err);
      // Fallback Demo Data
      setSummary({
        total: 42,
        active: 42,
                  ...
                  return (
                    <div key={a.id || i} className="fe-alert-row" style={{ '--alert-color': c }}>
                      ...
                      <div className="fe-alert-meta">
                        ...
                        <div className="fe-alert-type">{a.equipment_type || 'Drum Hose'}</div>
                      </div>
                    </div>
                  );
                });
                ...
                {pageSlice.map((item, i) => {
                  ...
                  return (
                    <div key={item.id || i} className="fe-table-row" onClick={() => openDetail(item)}>
                      ...
                      <span className="fe-table-type">{item.equipment_type || 'Drum Hose'}</span>
    ...
    <div className="fe-page">
      ...
      {detailLoading ? <Spinner /> : (
        <div className="fe-detail-grid">
          ...
          <div className="fe-detail-card fe-full">
            ...
            <div className="fe-identity-grid">
              {[
                ...
                { label: 'Type', val: u.equipment_type || 'Drum Hose' },
          ...
          {/* Physical Condition */}
          <div className="fe-detail-card">
            <SectionTitle>🔧 Mechanical Integrity</SectionTitle>
            <div className="fe-condition-pills">
              {[
                { label: 'Drum Rotation', val: u.rotation_status || 'OK' },
                { label: 'Hose Seal', val: u.seal_status || 'OK' },
                { label: 'Inlet Valve', val: u.inlet_status || 'OK' },
                { label: 'Mounting', val: u.mounting_status || 'OK' },
              ].map(pill => {
