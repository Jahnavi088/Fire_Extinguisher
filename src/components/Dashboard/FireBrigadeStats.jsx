...
const condColor = (v) =>
  v === 'OK' || v === 'READY' || v === 'OPERATIONAL' ? '#28a745'
    : (v === 'LOW_FUEL' || v === 'MAINTENANCE_DUE' || v === 'PARTIAL') ? '#FF9800'
      : (v === 'OFF_DUTY' || v === 'FAULTY' || v === 'UNAVAILABLE') ? '#dc3545'
        : '#666';
...
const KPI_CARDS = [
  ...
  { type: 'active', label: 'Ready for Dispatch', icon: '✅', color: '#28a745', key: 'active' },
  { type: 'needs-service', label: 'Under Repair', icon: '🔧', color: '#f59e0b', key: 'needs_service' },
  { type: 'expired', label: 'Critical Outage', icon: '⌛', color: '#8b5cf6', key: 'expired' },
  { type: 'due-inspection', label: 'Due Drill/Check', icon: '🚨', color: '#dc3545', key: 'due_inspection' },
];
...
const fetchByType = async (type) => {
  if (type === 'active') {
    const [activeData, upcomingData] = await Promise.all([
      ApiService.getEquipment({ module_id: 20, limit: 500, status: 'active' }),
      ApiService.getEquipment({ module_id: 20, limit: 500, status: 'upcoming' }),
    ]);
    return {
  ...
  const params = { module_id: 20, limit: 500 };
...
const Spinner = () => (
  <div className="fe-spinner">
    ...
    <span className="fe-spinner-text">Loading fire brigade data…</span>
  </div>
);
...
const ReadinessBar = ({ score }) => {
  ...
  return (
    <>
      <div className="fe-readiness-label">Operational Readiness Score</div>
...
/* ══════════════════════════════════════════════════════════════════════════ */
const FireBrigadeStats = ({ module, onBack, onRaiseWorkOrder }) => {
  ...
  const load = async () => {
    try {
      ...
      const [sum, alertsSum, alertsData] = await Promise.all([
        ApiService.getModuleSummary(20),
        ApiService.getAlertsSummary(),
        ApiService.getAlerts({ module_id: 20, limit: 100 }),
      ]);
      setSummary(sum);
      setAlertsSummary(alertsSum);
      setTopAlerts(alertsData.alerts || []);
    } catch (err) {
      console.error('API Load failed for Fire Brigade, using fallback:', err);
      setSummary({ total: 4, active: 3, upcoming: 1, needs_service: 0, expired: 0, due_inspection: 0, readiness_score: 91 });
      setAlertsSummary({ total_alerts: 1, level_1: { count: 1, label: 'Low', description: 'Refuel due' }, level_2: { count: 0, label: 'Med', description: 'Fault' }, level_3: { count: 0, label: 'High', description: 'Critical' } });
      setTopAlerts([]);
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        const mock = Array.from({ length: 4 }).map((_, i) => ({
          id: `fb_${i}`,
          sos_code: `BRIGADE-0${i + 1}`,
          equipment_type: 'Industrial Fire Engine',
          location_name: `Fire Station ${i + 1}`,
          building_name: 'Response Center',
          readiness_score: 95,
      ...
      <div className="fe-page">
        ...
        <div className="fe-panels">
          <div className="fe-panel">
            <div className="fe-panel-title">🔔 Brigade Readiness Alerts <span className="fe-panel-title-count">{totalAlerts}</span></div>
            <div className="fe-alert-list">
              {topAlerts.length === 0 ? <div className="fe-empty">No active brigade alerts.</div> : topAlerts.map((a, i) => (
          ...
          <div className="fe-panel">
            <div className="fe-panel-title">📊 Fleet Availability</div>
            {summary && (
              <>
          ...
          return (
            <div className="fe-list-body">
              <div className="fe-table">
                <div className="fe-table-head">{['SOS Code', 'Location', 'Building', 'Readiness', 'Next Review'].map(h => <span key={h} className="fe-table-head-cell">{h}</span>)}</div>
    ...
    <div className="fe-page">
      <div className="fe-header">
        ...
        <div className="fe-header-info">
          ...
          <div className="fe-header-sub">{u.equipment_type || 'Fire Brigade Unit'} · {u.location_name}</div>
        </div>
        {onRaiseWorkOrder && (
          <button
        ...
        <div className="fe-detail-grid">
          <div className="fe-detail-card fe-full"><div className="fe-section-title">Unit Identity</div>
            <div className="fe-identity-grid">
              ...
              <InfoRow label="Type" val={u.equipment_type || 'Response Vehicle'} />
          ...
          <div className="fe-detail-card"><div className="fe-section-title">📅 Maintenance Cycle</div>
            <InfoRow label="Last Dispatch" val={fmt(u.last_dispatch_date || u.installed_on)} />
            <InfoRow label="Last Inspection" val={fmt(u.last_inspection_date)} />
            <InfoRow label="Next Review" val={fmt(u.next_inspection_due)} color="#FF9800" />
          </div>
          <div className="fe-detail-card"><div className="fe-section-title">🔧 Operational Integrity</div>
            <div className="fe-condition-pills">
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.fuel_status || 'OK') + '44', background: condColor(u.fuel_status || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Fuel Level</span><span className="fe-condition-pill-value" style={{ color: condColor(u.fuel_status || 'OK') }}>{u.fuel_status || 'OPERATIONAL'}</span>
              </div>
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.pump_status || 'OK') + '44', background: condColor(u.pump_status || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Pump System</span><span className="fe-condition-pill-value" style={{ color: condColor(u.pump_status || 'OK') }}>{u.pump_status || 'READY'}</span>
              </div>
            </div>
...
export default FireBrigadeStats;
