...
const WorkOrders = ({ onBack, prefill, clearPrefill, allowedModules }) => {
  ...
  const loadOrders = async () => {
    setLoading(true);
    try {
      const [res, eqResRaw] = await Promise.all([
      ...
      if (allowedModules) {
        ...
        const allowedCodes = new Set(allowedModules.map(m => m.code || m.module_code));
