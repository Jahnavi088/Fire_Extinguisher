...
const fetchByType = (type) => {
  const params = { module_id: 28, limit: 200 };
...
const Spinner = () => (
  <div className="fe-spinner">
    ...
    <span className="fe-spinner-text">Loading muster point data…</span>
  </div>
);
...
const Pagination = ({ page, totalPages, total, pageSize, onPage }) => {
  ...
  return (
    <div className="fe-pagination">
      <span className="fe-page-info">Showing <strong>{start}–{end}</strong> of <strong>{total}</strong> points</span>
...
const ReadinessBar = ({ score }) => {
  ...
  return (
    <>
      <div className="fe-readiness-label">Safety Readiness Score</div>
  ...
  const load = async () => {
    try {
      ...
      const [sum, alertsSum, alertsData] = await Promise.all([
        ApiService.getModuleSummary(28),
        ApiService.getAlertsSummary(),
        ApiService.getAlerts({ module_id: 28, limit: 100 }),
      ]);
      setSummary(sum);
    ...
    return (
      <div className="fe-page">
        <div className="fe-header">
          ...
          <div className="fe-header-info"><div className="fe-header-title">Muster Point Safety Monitor</div></div>
          ...
          <div className="fe-header-search">
            <div className="fe-search-box">
              ...
              <input type="text" placeholder="Search point code..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="fe-search-input" />
            </div>
          </div>
        </div>
        ...
        <div className="fe-panels">
          ...
          <div className="fe-panel">
            <div className="fe-panel-title">📊 Deployment Readiness</div>
            {summary && (
              <>
                <div style={{ height: 160, width: '100%', marginTop: 5 }}>
                  <ResponsiveContainer>
                    <BarChart data={[
                      { name: 'Accessible', val: summary.active, color: '#28a745' },
    ...
    return (
      <div className="fe-page">
        <div className="fe-header">
          ...
          <div className="fe-header-info"><div className="fe-header-title">{listCfg.title}</div><div className="fe-header-sub">{listTotal} points found</div></div>
        </div>
        {listLoading ? <Spinner /> : (() => {
    ...
    <div className="fe-page">
      <div className="fe-header">
        ...
        <div className="fe-header-info">
          ...
          <div className="fe-header-sub">{u.equipment_type || 'Muster Point'} · {u.location_name}</div>
        </div>
        ...
        <div className="fe-detail-grid">
          <div className="fe-detail-card fe-full"><div className="fe-section-title">Point Identity</div>
            <div className="fe-identity-grid">
              ...
              <InfoRow label="Type" val={u.equipment_type || 'Assembly Point'} />
          ...
          <div className="fe-detail-card"><div className="fe-section-title">🔧 Operational Integrity</div>
            <div className="fe-condition-pills">
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.accessibility_status || 'OK') + '44', background: condColor(u.accessibility_status || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Access</span><span className="fe-condition-pill-value" style={{ color: condColor(u.accessibility_status || 'OK') }}>{u.accessibility_status || 'CLEAR'}</span>
              </div>
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.signage_status || 'OK') + '44', background: condColor(u.signage_status || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Signage</span><span className="fe-condition-pill-value" style={{ color: condColor(u.signage_status || 'OK') }}>{u.signage_status || 'VISIBLE'}</span>
              </div>
            </div>
...
export default MusterPointStats;
