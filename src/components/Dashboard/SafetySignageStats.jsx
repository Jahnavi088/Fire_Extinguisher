...
const KPI_CARDS = [
  ...
  { type: 'active', label: 'Compliant', icon: '✅', color: '#10b981', key: 'active' },
...
const fetchByType = async (type) => {
  if (type === 'active') {
    const [activeData, upcomingData] = await Promise.all([
      ApiService.getEquipment({ module_id: 62, limit: 500, status: 'active' }),
      ApiService.getEquipment({ module_id: 62, limit: 500, status: 'upcoming' }),
    ]);
    return {
  ...
  const params = { module_id: 62, limit: 500 };
...
/* ══════════════════════════════════════════════════════════════════════════ */
const SafetySignageStats = ({ module, onBack, onRaiseWorkOrder }) => {
  ...
  const load = async () => {
    try {
      ...
      const [sum, alertsSum, alertsData] = await Promise.all([
        ...
        ApiService.getAlertsSummary({ module_id: 62 }),                         // /alerts/summary
              ...
              <>
                <div style={{ height: 160, width: '100%', marginTop: 5 }}>
                  <ResponsiveContainer>
                    <BarChart
                      data={[
                        { name: 'Compliant', val: (summary.active || 0) + (summary.upcoming || 0), color: '#10b981' },
                ...
                <div className="fe-fleet-legend" style={{ marginTop: 20 }}>
                  {[
                    { label: 'Compliant', val: (summary.active || 0) + (summary.upcoming || 0), color: '#10b981' },
