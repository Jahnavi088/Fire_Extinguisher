...
const condColor = (v) =>
  v === 'OK' || v === 'READY' || v === 'NORMAL' ? '#28a745'
    : (v === 'LOW_PRESSURE' || v === 'DIRTY' || v === 'STAINED') ? '#FF9800'
      : (v === 'NO_FLOW' || v === 'LEAKING' || v === 'CORRODED') ? '#dc3545'
        : '#666';
...
const fetchByType = async (type) => {
  if (type === 'active') {
    const [activeData, upcomingData] = await Promise.all([
      ApiService.getEquipment({ module_id: 60, limit: 500, status: 'active' }),
      ApiService.getEquipment({ module_id: 60, limit: 500, status: 'upcoming' }),
    ]);
    return {
  ...
  const params = { module_id: 60, limit: 500 };
...
const Spinner = () => (
  <div className="fe-spinner">
    ...
    <span className="fe-spinner-text">Loading chemical shower data…</span>
  </div>
);
...
/* ══════════════════════════════════════════════════════════════════════════ */
const ChemicalShowerStats = ({ module, onBack, onRaiseWorkOrder }) => {
  ...
  const load = async () => {
    try {
      ...
      const [sum, alertsSum, alertsData] = await Promise.all([
        ApiService.getModuleSummary(60),
        ApiService.getAlertsSummary(),
        ApiService.getAlerts({ module_id: 60, limit: 100 }),
      ]);
      setSummary(sum);
      setAlertsSummary(alertsSum);
      setTopAlerts(alertsData.alerts || []);
    } catch (err) {
      console.error('API Load failed for Chemical Shower, using fallback:', err);
      setSummary({ total: 8, active: 8, upcoming: 0, needs_service: 0, expired: 0, due_inspection: 0, readiness_score: 100 });
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        const mock = Array.from({ length: 4 }).map((_, i) => ({
          id: `cs_${i}`,
          sos_code: `CS-500${i + 1}`,
          equipment_type: 'Chemical Decontamination Shower',
          location_name: `HAZMAT Zone ${i + 1}`,
          building_name: 'Chemical Storage',
          readiness_score: 100,
    ...
    return (
      <div className="fe-page">
        <div className="fe-header">
          ...
          <div className="fe-header-info"><div className="fe-header-title">Chemical Shower Fleet Monitor</div></div>
        ...
        <div className="fe-panels">
          <div className="fe-panel">
            <div className="fe-panel-title">🔔 Decontamination System Alerts <span className="fe-panel-title-count">{totalAlerts}</span></div>
    ...
    <div className="fe-page">
      <div className="fe-header">
        ...
        <div className="fe-header-info">
          ...
          <div className="fe-header-sub">{u.equipment_type || 'Chemical Shower'} · {u.location_name}</div>
        </div>
        {onRaiseWorkOrder && (
          <button
        ...
        <div className="fe-detail-grid">
          <div className="fe-detail-card fe-full"><div className="fe-section-title">Unit Identity</div>
            <div className="fe-identity-grid">
              ...
              <InfoRow label="Type" val={u.equipment_type || 'Decon Shower'} />
          ...
          <div className="fe-detail-card"><div className="fe-section-title">📅 Maintenance Cycle</div>
            ...
            <InfoRow label="Anti-Corrosion Check" val={u.corrosion_check || 'OK'} />
          </div>
          <div className="fe-detail-card"><div className="fe-section-title">🔧 Functional Integrity</div>
            <div className="fe-condition-pills">
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.flow_status || 'OK') + '44', background: condColor(u.flow_status || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Flow Rate</span><span className="fe-condition-pill-value" style={{ color: condColor(u.flow_status || 'OK') }}>{u.flow_status || 'NORMAL'}</span>
              </div>
              <div className="fe-condition-pill" style={{ borderColor: condColor(u.structure_status || 'OK') + '44', background: condColor(u.structure_status || 'OK') + '12' }}>
                <span className="fe-condition-pill-label">Structure</span><span className="fe-condition-pill-value" style={{ color: condColor(u.structure_status || 'OK') }}>{u.structure_status || 'READY'}</span>
              </div>
            </div>
...
export default ChemicalShowerStats;
